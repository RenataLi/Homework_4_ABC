...
mov rbp, rsp
    ...
    ; Сохранени принятых аргументов
    mov     [.ptrian], rdi        ; сохраняется адрес автобуса
    mov     [.FILE], rsi          ; сохраняется указатель на файл
...
section .bss
    .prect  resq  1
    .FILE   resq  1       ; временное хранение указателя на файл
    .p      resq  1       ; вычисленное расстояние грузовика
section .text
push rbp
mov rbp, rsp

    ; Сохранени принятых аргументов
    mov     [.prect], rdi          ; сохраняется адрес грузовика
    mov     [.FILE], rsi          ; сохраняется указатель на файл
   ...
   mov     rdi, [.FILE]
    mov     rsi, .outfmt        ; Формат - 2-й аргумент
    mov     rax, [.prect]      ; адрес треугольника
    mov     rcx,[rax]          ; 0 int consumption
    mov     rdx, [rax+4]        ; +4 int capacity
    mov     r8, [rax+8]       ; +4+4 int lifting capacity
   ; movsd   xmm0, [rax+12]    ; +4+4+4 double distance
    movsd   xmm0, [.p]
    mov     rax, 1              ; есть числа с плавающей точкой
    call    fprintf
